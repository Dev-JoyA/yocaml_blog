...
type t = { source : Path.t; target : Path.t; server_root : Path.t }

let make ?(source_folder = Path.rel []) ?(target_folder = Path.rel [ "_www" ])
    ?(server_root = Path.abs []) () =
  { source = source_folder; target = target_folder; server_root }
...
module Target = struct
  ...
  let cache r = Path.(target r / ".cache")
  let images r = Path.(target r / "images")
  let style_css r = Path.(target r / "style.css")
